...
# PART 1: EXTERNAL ENGINE IMPORTS (350+ LINE REQUIREMENT)
...
# PART 2: PAGE CONFIG & CSS STYLING (HARDCORE THEME)
...
# PART 3: UNIVERSAL HEADER & MISSION
...
- **Analyzes** universal transaction data (CSV, Excel, or structured exports)
- **Identifies** hidden drivers of cash burn
- **Predicts** exact liquidity runway
...
if uploaded_file:
    try:
        ...
        if df.empty:
            ...
            # PART 6: METRICS & KPI DASHBOARD (HARDCORE LOGIC)
            ...
            # PART 7: VISUAL ANALYSIS, SEARCH, & ACTION PLAN
            st.divider()
            ...
            st.plotly_chart(px.line(f_df, x="date", y="closing_cash", title="Predicted Liquidity Position"), use_container_width=True)

            # 🔍 STRATEGIC AUDIT (QUESTION SEARCH)
            st.divider()
            ...
            with col_pa:
                ...
                if burn_mult > 1.5:
                    ...
                    st.markdown(f"1. **Stable Efficiency:** Burn Multiple is {burn_mult:.2f}x. Focus on scaling current unit economics.")
                st.markdown(f"2. **Marketing Intensity:** Ads represent {metrics.get('ad_spend_pct', 0)*100:.1f}% of outflows.")
                st.markdown(f"3. **Survival Horizon:** Estimated runway is {metrics['runway_months']} months.")
            with col_pb:
            ...
            def generate_pdf():
                ...
                with PdfPages(buf) as pdf:
                    ...
                    txt = (f"EXECUTIVE FINANCIAL FLASH REPORT\n"
                           ...
                           f"Burn Multiple: {burn_mult:.2f}x")
